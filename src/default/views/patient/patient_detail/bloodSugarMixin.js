...
export default {
  ...
  methods: {
      ...
      this.$api.bloodSugarRecords
        ...
        .then((res) => {
          ...
          this.list.forEach(ele => {
            const ite = res.data.results.find(el => el.time_period == ele.value);
            if (ite) {
                dataInfo[ele.value] = ite.value
            } else {
                dataInfo[ele.value] = "--"
            }
          })
          this.todayWeightRecordTable = [dataInfo]
        })
        .catch((err) => {
          this.$message.error(JSON.stringify(err));
        });
    ...
    validateTodayWeightRecordForm() {
      ...
      if (this.time_period === "") {
        this.$message.error("请选择测量时段");
        return false;
      }
      if (this.value === "") {
        this.$message.error("请输入血糖值");
        return false;
      }
      if (Number(this.value).toString() === "NaN") {
        this.$message.error("血糖值只能是数字");
        return false;
      }
      return true;
    },
    ...
    async renderWeightChart() {
      ...
      this.weightChartOptions.series[1].data = []
      ...
      switch (this.weightKindRadio) {
        case "周": {
          ...
          this.weightChartOptions.series[1].data = new Array(7)
        ...
        case "月": {
          ...
          this.weightChartOptions.series[1].data = new Array(4);
          data = await this.getPatientStatByChart("day", start, end);
          break;
        }
        case "年": {
          ...
          this.weightChartOptions.series[1].data = new Array(12);
        ...
        case "自定义": {
          ...
          this.weightChartOptions.series[1].data = new Array(j);
          data = await this.getPatientStatByChart("day", start, end);
          break;
        }
      }
        ...
        case "周": {
          this.weightChartOptions.xAxis[0].data.forEach((item, index) => {
            weightStats.forEach((stat) => {
              ...
              if (statDate === item) {
                this.weightChartOptions.series[0].data[index] = stat.before_avg;
                this.weightChartOptions.series[1].data[index] = stat.avg;
              }
            });
          });
          break;
