    ...
    return {
      weightChartOptions: {
        tooltip: {
          ...
          // },
          formatter: (params) => {
            const item = this.moodList[this.moodList.length - params[0].data]
            return params[0].name + "：" + (item ? item.name : "")
          }
        },
        grid: {
          left: 60
      ...
      weightHistoryRadio: "心情",
      todayWeightRecord: {},
      todayWeightRecordTable: [],

      todayWeight: "", // 今日心情
      todayInitWeight: "", // 初始心情
      todayTargetWeight: "", // 目标心情
        ...
        case "周": {
          this.weightChartOptions.xAxis[0].data.forEach((item, index) => {
            weightStats.forEach((stat) => {
              ...
              if (statDate === item) {
                const k = this.moodList.findIndex(
                  (ele) => ele.type == stat.emotion
                );
                this.weightChartOptions.series[0].data[index] =
                  this.moodList.length - k;
              }
            });
          });
          break;
            ...
            weightStats.forEach((stat) => {
              ...
              if (next) {
                if (
                  ...
                  const k = this.moodList
                    .reverse()
                    .findIndex((ele) => ele.type == stat.emotion);
                  this.weightChartOptions.series[0].data[index] =
                    this.moodList.length - k;
                }
              } else {
