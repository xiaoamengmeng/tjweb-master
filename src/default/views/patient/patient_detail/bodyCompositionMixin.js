...
export default {
  data() {
    return {
      ...
      weightHistoryRadio: "BMI",
      todayWeightRecord: {},
      todayWeightRecordTable: [],

      todayWeight: "", // 今日BMI
      todayInitWeight: "", // 初始BMI
      todayTargetWeight: "", // 目标BMI
  ...
  methods: {
    ...
    getBloodPressureRecords() {
      ...
      this.$api.bodyComposition
        ...
        .then((res) => {
          ...
          if (this.bloodPressureHistoryTableData.length > 0) {
            this.selectedValue = this.bloodPressureHistoryTableData[0].id
          }
        })
        .catch((err) => {
          this.$message.error(JSON.stringify(err));
    ...
    bodyChange() {
      switch (this.bodyRadio) {
        case "BMI":
          this.weightChartOptions.legend.data[0] = "BMI";
          this.weightChartOptions.legend.selected = {
            BMI: true,
          };
          this.weightChartOptions.yAxis[0].name = "BMI";
          this.weightChartOptions.series[0].name = "BMI";
          break;
        case "体脂率":
          this.weightChartOptions.legend.data[0] = "体脂率";
          this.weightChartOptions.legend.selected = {
            体脂率: true,
          };
          this.weightChartOptions.yAxis[0].name = "体脂率";
          this.weightChartOptions.series[0].name = "体脂率";
          break;
        case "内脏脂肪":
          this.weightChartOptions.legend.data[0] = "内脏脂肪";
          this.weightChartOptions.legend.selected = {
            内脏脂肪: true,
          };
          this.weightChartOptions.yAxis[0].name = "内脏脂肪";
          this.weightChartOptions.series[0].name = "内脏脂肪";
          break;
        case "肌肉量":
          this.weightChartOptions.legend.data[0] = "肌肉量";
          this.weightChartOptions.legend.selected = {
            肌肉量: true,
          };
          this.weightChartOptions.yAxis[0].name = "肌肉量";
          this.weightChartOptions.series[0].name = "肌肉量";
          break;
      }
      ...
      this.$api.weightRecords
        ...
        .then((res) => {
          if (this.weightDetailDialogVisible) {
            ...
            if (this.weightHistoryRadio === "BMI") {
              this.getWeightRecords();
            }
          }
        })
    ...
    deleteWeightRecord(row) {
      this.$confirm(`确定此条BMI记录吗？`, "删除BMI记录", {
        confirmButtonText: "删除",
        cancelButtonText: "取消",
        type: "error",
      })
        .then(() => {
          this.$api.weightRecords
            .delete(row.id)
            .then((res) => {
              this.$message.success("删除BMI记录成功");
              this.getWeightRecords();
              this.getPatientStat();
            })
            .catch((err) => {
