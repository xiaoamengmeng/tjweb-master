<template>
  <div class="panel scroll">
    ...
    <m-button v-auth="'scaleManage.create'" class="common-add-btn" icon="el-icon-plus" @click="toSetPage('create')">新建量表</m-button>
    <div class="card">
        ...
        >
          <div class="scale-item-row-info">
            <div class="scale-title">量表名称：{{ scale.name }}
              <el-tag v-if="scale.start_time&&dayjs().isAfter(dayjs(scale.end_time))" type="danger" effect="dark" style="margin-left:5px;">已过期</el-tag>
              <el-tag v-if="scale.start_time&&dayjs().isBefore(dayjs(scale.start_time))" type="warning" effect="dark" style="margin-left:5px;">未开始</el-tag>
            </div>
          ...
          <div class="scale-item-row-operate">
            <div class="btn-group">
              <m-button v-auth="'scaleManage.update'" type="iron" w="100px" h="32px" v-if="scale.can_edit" @click="toSetPage('edit', scale)">编辑量表</m-button>
              <m-button v-auth="'scaleManage.info'" type="iron" w="100px" h="32px" @click="toPreviewPage(scale)">预览量表</m-button>
              <!-- <m-button v-auth="'scaleManage.questionnaire'" type="iron" w="100px" h="32px" v-if="scale.can_edit" @click="toSettingPage(scale)">问卷设置</m-button> -->
              <m-button v-auth="'scaleManage.answer'" type="iron" w="100px" h="32px" @click="toRecordPage(scale)">答题记录</m-button>
              <m-button v-auth="'scaleManage.answerStatistics'" type="iron" w="100px" h="32px" @click="toStatPage(scale)">答题统计</m-button>
            </div>
            <div class="icon-btn-group">
<!--              <el-link @click="shareClick(scale)">-->
<!--                <i class="el-icon-position"></i>分享-->
<!--              </el-link>-->
              <el-link v-auth="'scaleManage.copy'" @click="copyClick(scale)">
                <i class="el-icon-document-copy"></i>复制
              </el-link>
              <el-link v-auth="'scaleManage.delete'" @click="deleteClick(scale)">
...
export default {
  ...
  methods: {
    ...
    statusChange(scale){
      // console.log(scale)
      this.$api.scale.patch(scale.id,{status:scale.status}).then(()=>{}).catch(err=>{
