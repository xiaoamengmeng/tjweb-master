    ...
    <div class="card">
      ...
      >
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="参照物图片" prop="comparison_image_path">
              <m-upload
                class="upload-box"
                :file-list.sync="uploadFile.comparisonFileList"
                :image-url.sync="comparisonImgSrc"
                :image-path.sync="form.comparison_image_path"
                accept=".jpg,.jpeg,.png,.gif"
                success-tips="上传图片成功"
                :readonly="isViewMode"
              ></m-upload>
              <div class="comparison_image" v-if="isViewMode && (comparisonImgSrc == null || comparisonImgSrc== '')"></div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  data() {
    ...
    const validComparisonImage = (rule, value, callback) => {
      if (this.type === 'create' && this.form.comparison_image_path === '') {
        callback(new Error('请上传参考图片'))
      }
      if (this.type === 'edit' && this.comparisonImgSrc === '') {
        callback(new Error('请上传参考图片'))
      }
      callback()
    }
    return {
      ...
      rules: {
        ...
        // comparison_image_path: [
        //   { required: true, validator: validComparisonImage, message: '请上传参考图片', trigger: 'blur' },
        // ],
      },
      imgSrc: '',
      comparisonImgSrc: '',
      ...
      if (this.type === 'create') {
        ...
        this.$api.food.getDetail(data.id)
          .then((res) => {
            ...
            this.comparisonImgSrc = res.data.comparison_image
            this.form.image_path = res.data.image_path
            this.form.comparison_image_path = res.data.comparison_image_path
